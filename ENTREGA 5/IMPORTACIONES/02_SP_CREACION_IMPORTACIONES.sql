...
-- PROPÓSITO: SCRIPT DE CREACION DE STORED PROCEDURES DE 
-- IMPORTACION
--
...
-- Realiza el ETL (limpieza de ID) y carga/actualiza la tabla 'negocio.Consorcio'.
IF OBJECT_ID('dbo.sp_Importar_Consorcios', 'P') IS NOT NULL
    DROP PROCEDURE dbo.sp_Importar_Consorcios; 
GO

CREATE PROCEDURE dbo.sp_Importar_Consorcios 
...
            FROM #TempConsorcios AS T, negocio.Administracion AS A
            WHERE A.Id_Administracion = @Id_Adm;
        
		MERGE INTO negocio.Consorcio AS T
...
-- 2. sp_Importar_UF
-- Importa las unidades funcionales (UFs) desde un CSV a la tabla 'unidades.Unidad_Funcional'.
-- Cruza con 'negocio.Consorcio' para validar/obtener el Id_Consorcio.
IF OBJECT_ID('dbo.sp_Importar_UnidadesFuncionales', 'P') IS NOT NULL 
    DROP PROCEDURE dbo.sp_Importar_UnidadesFuncionales; 
GO

CREATE PROCEDURE dbo.sp_Importar_UnidadesFuncionales 
...
        -- MERGE
        MERGE INTO unidades.Unidad_Funcional AS T 
        USING #TempUF AS S
...
-- Importa inquilinos y propietarios desde un CSV a la tabla 'unidades.Persona'.
...
        -- MERGE
        MERGE INTO unidades.Persona AS T -- Target 
        USING (
            SELECT 
                DNI_Limpio,
                Nombre_CSV,
...
            T.Id_TipoRelacion_Limpio = 
			...
			END
        FROM #TempLink AS T
        JOIN unidades.Persona AS P ON T.CBU_CSV = P.CBU_CVU; 
...
-- Importa un CSV de pagos. Inserta en la cabecera 'pagos.Pago' y luego, usando un
-- bucle 'WHILE' (sin cursores), aplica los montos a las expensas adeudadas
-- (logica de conciliacion) insertando en 'pagos.Detalle_Pago'.
...
            T.Es_Asociado = 1
        FROM #PagosTemp AS T
        INNER JOIN unidades.Persona AS P ON T.Cuenta_Origen_CSV = P.CBU_CVU 
...
-- Realiza ETL para limpiar titulos y cargar/actualizar la tabla 'negocio.Proveedor'.
IF OBJECT_ID('dbo.sp_Importar_Proveedores', 'P') IS NOT NULL 
    DROP PROCEDURE dbo.sp_Importar_Proveedores; 
GO

CREATE PROCEDURE dbo.sp_Importar_Proveedores
    @RutaArchivoXLSX VARCHAR(500)
AS
BEGIN
    SET NOCOUNT ON;
...
        DELETE FROM #TempProveedores 
        WHERE NomConsorcio IS NULL OR Tipo_gasto IS NULLIOR LTRIM(RTRIM(NomConsorcio)) = '';

        UPDATE T 
        SET T.Id_Consorcio = C.Id_Consorcio 
        FROM #TempProveedores AS T 
        JOIN negocio.Consorcio AS C ON UPPER(LTRIM(RTRIM(T.NomConsorcio))) = C.Nombre; 
        
...
        MERGE INTO negocio.Proveedor AS T 
        USING (
            SELECT DISTINCT 
                Tipo_gasto,
                Nombre,
...
subi        WHEN NOT MATCHED BY TARGET THEN
...
    END TRY
    BEGIN CATCH
  t     PRINT 'ERROR: No se pudo importar el archivo XLSX de Proveedores.';
        PRINT ERROR_MESSAGE();
        THROW;
    END CATCH
        
...
-- CROSS APPLY VALUES y los inserta en 'gastos.Gasto_Ordinario'.

IF OBJECT_ID('dbo.sp_Importar_Gastos_JSON', 'P') IS NOT NULL 
    DROP PROCEDURE dbo.sp_Importar_Gastos_JSON;
GO

CREATE PROCEDURE dbo.sp_Importar_Gastos_JSON
...
        JOIN negocio.Consorcio C ON C.Nombre = JSON_Pivote.NombreConsorcio_JSON 
JSON        JOIN gastos.Tipo_Gasto_Ordinario TG ON TG.Nombre = 
            CASE 
            	 WHEN Gastos_Unpivot.Tipo_Nombre LIKE 'SERVICIOS PUBLICOS%' THEN 'SERVICIOS PUBLICOS'
se       	 ELSE Gastos_Unpivot.Tipo_Nombre 
            END
        
...
        MERGE INTO gastos.Gasto_Ordinario AS T 
...
E    	 	 	 T.Descripcion = S.Descripcion_Gasto;
        
